...
$infopaypal='';
	...
	if(isset($_SESSION['infopaypal'])){
        ...
        */
		...
		$order_info[] = "HQ-Online Version 2".PHP_EOL;
		$order_info[] = "IP : " . $info['ip_user'] . PHP_EOL;
		$order_info[] = "Date : " . $info['date_order'] . PHP_EOL;
		$order_info[] = "Zahlung : " . $info['type_payment'] . PHP_EOL;
		$order_info[] = "Benutzername : " . $info['email_login'] . PHP_EOL;
		
		$order_info[] = "Firma : " . $info['company'] . PHP_EOL;
		$order_info[] = "Abteilung : " . $info['office'] . PHP_EOL;
		$order_info[] = "Anrede : " . $info['sex'] . PHP_EOL;
		$order_info[] = "Vorname : " . $info['lastname'] . PHP_EOL;
		$order_info[] = "Nachname : " . $info['firstname'] . PHP_EOL;
		$order_info[] = "Strasse : " . $info['stress'] . PHP_EOL;
		$order_info[] = "PLZ : " . $info['postalcode'] . PHP_EOL;
		$order_info[] = "Ort : " . $info['region'] . PHP_EOL;
		
		$order_info[] = "Hausnummer : " . $info['numberhouse'] . PHP_EOL;
		$order_info[] = "Vorwahl : " . $info['first_number'] . PHP_EOL;

        $trimed_phone = preg_replace('/\D/', '', $info['phone']);
        $order_info[] = "Telefon : " . $trimed_phone . PHP_EOL;

        $order_info[] = "Email : " . $info['email'] . PHP_EOL;

        $info['noteposition']=preg_replace('/\n++/', '. ', $info['noteposition']);
        $info['noteposition']=preg_replace('/\r++/', '. ', $info['noteposition']);

		$order_info[] = "Hinterhof/Etage : " . $info['noteposition'] . PHP_EOL;

        $info['note']=preg_replace('/\n++/', '. ', $info['note']);
        $info['note']=preg_replace('/\r++/', '. ', $info['note']);

		$order_info[] = "Besonderheiten/Lieferzeitpunkt : " . $info['note'] . PHP_EOL;
		$order_info[] = "Entfernung : " . $info['distance_deliver'] . PHP_EOL;
		...
		foreach($ar as $food)
		{
            ...
            $order_info[] = "¶¶¶*¶¶¶" . $food['plu'] . "¶¶¶*¶¶¶" . $food['qty'] . "¶¶¶*¶¶¶" . $price_food. "¶¶¶*¶¶¶" .$beilage_food."¶¶¶*¶¶¶".$food['note']. PHP_EOL;
		}
        //$order_info[] = "------------------------------\n";

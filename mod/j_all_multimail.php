...
Class j_all extends  db{
	...
	// insert đơn hàng web
	function history_odrer($idUser,$Qty,$Total,$Day,$Address){

		$sql="INSERT INTO history_order(`idUser`,`Qty`,`Total`,`Day`,`Address`) VALUES ($idUser,'$Qty','$Total','$Day','$Address')";
	...
	function sendmail($TO_EMAIL, $subject, $body){
		...
		if($config['SMTP_USE_SV']==0){
			$mail->Host =$config['SMTP_SERVER_Y'];
			$mail->Username =$config['SMTP_USER_Y'] ;  
			$mail->Password = $config['SMTP_PASSWORD_Y'];
			$mail->SMTPSecure = $config['SMTPSecure_Y']; // Giao thức SSL
			$mail->Port = $config['SMTP_SERVER_PORT_Y']; // cổng SMTP
			$mail->From = $config['SMTP_USER_Y']; // mail người gửi  
		}elseif($config['SMTP_USE_SV']==2){
			$mail->Host =$config['SMTP_SERVER_WM'];
			$mail->Username =$config['SMTP_USER_WM'];  
			$mail->Password = $config['SMTP_PASSWORD_WM'];
			$mail->SMTPSecure = $config['SMTPSecure_WM']; // Giao thức SSL
			$mail->Port = $config['SMTP_SERVER_PORT_WM']; // cổng SMTP
			$mail->From = $config['SMTP_USER_WM']; // mail người gửi  
		}

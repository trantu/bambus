...
/***Button note mon an order mon an khong co an them **/
	...
	//event shownode hiển thị ghi chú
	...
	function shownote(parent_notes){ 
		var parent_note=parent_notes;	
		 var idsp_get_note = parent_note.find('.note_change_idsp').attr('idsp');// Lấy idsp
		 
		 	$.ajax({
		 	url:"index.php?mod=get_note",
		 	type:"POST",
		 	data:{idsp_get_note:idsp_get_note},
		 	success:function(sms){		 	
		 		var str = sms.replace(/\r\n/g,"").replace(/\n/g,"");		 	
		 		parent_note.find('.note-text').val(str);
		 		}
		 	})
		 		
		$(parent_note).find('.note_ones').css('display','block');
		$('.form-box-overlay').css('display','block'); // Hiện backgroud đen
	...
	function remove_item(idSP,STTPP,this_item,iconmoney,divdels,price){
		...
		$.ajax({	
			...
			success:function(smgs){
                ...
                var trimed_idSP = idSP.replace(/ /g, '.');
				var sls=$(parent_d).find('.quantity'+ trimed_idSP).text();
				sls=parseInt(sls);
				var slsnow=sls-1;

				if(slsnow>0){
					$(parent_d).find('.quantity'+ trimed_idSP).html(slsnow);
					var pr=slsnow;
					var pri_ce=price;
					...
					$(parent_d).find('.price'+ trimed_idSP).html(iconmoney + pricess);
				}
				if(slsnow==0) {
					divdel.remove();
					$.ajax({	
	...
	function add_item(idSP,STTPP,this_item,iconmoney,price){
		...
		$.ajax({	
			...
			success:function(smgs){
				...
				var sls=$(parent_a).find('.quantity'+ trimed_idSP).text();
				sls=parseInt(sls);
				var slsnow=sls+1;

				$(parent_a).find('.quantity'+trimed_idSP).html(slsnow);
				var pr=slsnow;
				var pri_ce=price;
				pri_ce=pri_ce.replace(',','.');
				pri_ce=parseFloat(pri_ce);
...
//                $('.form-box-overlay').html('');
//
//                // KKK: case of adding a line
//                //if(data.indexOf('0,00') == -1)
//                data = parseInt(data);
//                if(data >= 9876543210)
//                {
//                    $('#order_summary').append('<p id="' + data + '" style="display:block;border-bottom: 1px dotted #9AB5A7;color: #005a00">_____</p>');
//                    //$('#order_summary').append('<div id="' + data + '" style="display:block;height:2px;border-bottom: 1px dotted #9AB5A7;color: #005a00"><ul><li></li><ul></div>');
//                }
//            }
//        });
//
	...
	function add_item_cart(idSP,STTPP,this_item,iconmoney,price){
		...
		$.ajax({	
			...
			success:function(smgs){
				...
				var prices=(pr*pri_ce).toFixed(2);
				var pricess=prices.replace('.',',');
				$(parent_a).parent().find('.price'+trimed_idSP).html( iconmoney+'' +pricess);
		...
		$.ajax({	
			...
			success:function(smgs){
				...
				if(slsnow>0){
					...
					var prices=(pr*pri_ce).toFixed(2);
					var pricess=prices.replace('.',',');
					$(parent_d).parent().find('.price'+trimed_idSP).html( iconmoney+'' +pricess);
				}
				if(slsnow==0) {
					divdel.remove();
					$.ajax({	
